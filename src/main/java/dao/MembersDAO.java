		...
		try (Connection con = this.getConnection(); PreparedStatement pstat = con.prepareStatement(sql)) {
			...
			try (ResultSet rs = pstat.executeQuery()) {
				if (rs.next()) {
					...
					return new MembersDTO(userSeq, userId, userPwd, userName, nickName, phone, email, gender, signout,
							birth_date, join_date, upd_date, adminKey, tempCode);
				}
			}
		}
		return null;
		...
		try (Connection con = this.getConnection(); PreparedStatement pstat = con.prepareStatement(sql)) {
			...
			try (ResultSet rs = pstat.executeQuery()) {
				if (rs.next()) {
					...
					String email = rs.getString("email");
					Timestamp join_date = rs.getTimestamp("join_date");
					return new MembersDTO(0, userId, null, userName, nickName, phone, email, null, null, null,
							join_date, null, 0, null);
