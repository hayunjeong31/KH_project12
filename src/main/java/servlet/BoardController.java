   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try {
         ...
         if (cmd.equals("/list.board")) {
            ...
            List<BoardWithReplyCount> list = dao.selectNtoM(
         ...
         // 게시글 검색
             ...
             List<BoardWithReplyCount> list = dao.searchPosts(keyword, option, 
                     cpage * BoardConfig.RECORD_COUNT_PER_PAGE - (BoardConfig.RECORD_COUNT_PER_PAGE - 1), 
                     cpage * BoardConfig.RECORD_COUNT_PER_PAGE);
         ...
         // 게시글 보기 
                  else if(cmd.equals("/mydetail.board")) {
                         String loginID = (String)request.getSession().getAttribute("loginID");
                     int seq = Integer.parseInt(request.getParameter("seq"));
                     BoardDTO dto = dao.getinfo(seq);
                     int categorySeq = 1;
                     ...
                     dao.increaseViewCount(seq); 
                     request.getRequestDispatcher("/board/mydetail.jsp").forward(request, response);
                     
                  }   
         
         
         // 게시글 수정하기 페이지로 넘어가기.
                  else if (cmd.equals("/myedit.board")) {
                         int seq = Integer.parseInt(request.getParameter("seq"));
                         BoardDTO dto = dao.getinfo(seq);
                         List<FilesDTO> list = f_dao.selectAll(seq,1);
                         request.setAttribute("list", list);
                         request.setAttribute("dto", dto);
                         request.getRequestDispatcher("/board/myedit.jsp").forward(request, response);
                         
                         //
                         

                     }   // 게시글 수정하기 
                  else if (cmd.equals("/myupdate.board")) {
                     ...
                     response.sendRedirect("/mydetail.board?seq=" +parent_seq);
                     
                     }   //게시글 삭제하기 
                  else if (cmd.equals("/mydelete.board")) {
                         int seq = Integer.parseInt(request.getParameter("seq"));
                         dao.delete(seq);
                         response.sendRedirect("/myfreepostlist.board");
                     }
