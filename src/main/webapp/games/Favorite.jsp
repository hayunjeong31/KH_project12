...
.board-left {
	flex: 1;
	background-color: rgba(4, 20, 241, 0.684);
	color: white;
	display: flex;
...
.favorite-actions .btn-edit:hover {
	color: rgb(249, 63, 76);
}

.favorite-actions .btn-delete:hover {
	color: #ff0000;
}
...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.length === 0) {
                favoriteList.append('<li>즐겨찾기 목록이 없습니다.</li>');
            } else {
                data.forEach(function(favorite) {
                    var favoriteItem = $('<li class="favorite-item"></li>');
                    var favoriteTitle = $('<p class="favorite-title"></p>').text(favorite.title);
                    var favoriteDescription = $('<p class="favorite-description"></p>').text(favorite.description);
                    var favoriteLink = $('<a href="#" class="favorite-link"></a>').text(favorite.pageUrl);
                    var postSeq = $('<p class="post-seq"></p>').text(': 게임 페이지 링크');

                    favoriteLink.click(function() {
                        window.open(favorite.pageUrl, '_blank');
                    });

                    favoriteItem.append(favoriteTitle);
                    favoriteItem.append(favoriteDescription);
                    favoriteItem.append(favoriteLink);
                    favoriteItem.append(postSeq);
                    favoriteList.append(favoriteItem);
                });
            }
        },
        error: function() {
            $('.favorite-list').append('<li>즐겨찾기 목록을 불러오는 중 오류가 발생했습니다.</li>');
        }
    });
});
</script>
